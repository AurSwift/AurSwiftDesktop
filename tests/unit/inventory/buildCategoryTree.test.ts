...
describe("buildCategoryTree Performance", () => {
  ...
  describe("Performance Benchmarks", () => {
    ...
    it("should verify O(n) complexity by comparing 1k vs 10k", () => {
      ...
      // O(n) means 10x data => roughly ≤10x time. Use 60x to allow CI variance
      // (O(n²) would be ~100x and must fail)
      const scalingFactor = duration10k / Math.max(duration1k, 0.01);
